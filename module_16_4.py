...
async def post_user(
        username: Annotated[str, Path(min_length=3, max_length=20, description='Enter username', example="Irina")],
        age: Annotated[int, Path(ge=18, le=90, description="Enter age", example="33")]
) -> str:
...
async def update_user(user_id: int, new_username: str,  new_age: int) -> str:
    for user in users:
        if user.id == user_id:
